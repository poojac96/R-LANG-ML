...
#NOTE : Each column can said to be a variable & Each row can said to be a record.
a <- c(10,20,30,40)
...
head(df) #returns top 6 rows
head(df,n=3) #returns top 3 rows
tail(df) #returns last 6 rows
View(df)
class(df) # DF
summary(df) #summary
#Slice Data Frame: 
